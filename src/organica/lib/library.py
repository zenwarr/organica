...
from organica.lib.filters import Wildcard, generateSqlCompare
...
class Library(QObject, Lockable):
    ...
    def createTagClass(self, name, value_type = TagValue.TYPE_TEXT, is_hidden = False):
        ...
        with self.lock:
            ...
            # check if we have another class with this name
    ...
    def flushTagClass(self, new_class):
        with self.lock:
            ...
            if not old_class:
                return self.createTagClass(new_class.name, new_class.valueType,
                                           new_class.hidden)
    ...
    def _checkTagClass(tag_class):
        ...
        if not (0 <= tag_class.valueType < TagValue.TYPE_MAXIMAL):
    ...
    def createTag(self, tag_class, value):
        ...
        with self.lock:
            if isinstance(tag_class, str):
                tag_class = self.getOrCreateTagClass(tag_class)
            elif isinstance(tag_class, Identity):
                tag_class = self.tagClass(tag_class)
            else:
                self.flush(tag_class)

            tag = Tag(tag_class, tag_value)

            self._checkTag(tag_class, value)

            # check if we already have duplicate of this tag
            if self.tag(TagFilter().tagClass(tag_class).value(value)):
                raise ValueError('tag with same class and value already exists')

            with self.transaction() as c:
                c.execute('insert into tags(class_id, value_type, value) ' \
                          'values(?, ?, ?)', (int(tag_class.id),
                                              int(tag_class.valueType),
                                              tag_value.databaseForm()))
    ...
    def flushTag(self, new_tag):
        with self.lock:
            ...
            if not old_tag:
                return self.createTag(new_tag.tagClassName, new_tag.tagValue)
            else:
                ...
                if old_tag.tagClass != new_class.tagClass:
        ...
        def object(self, query):
            ...
            if query.id in self._objects:
                return copy(self._objects[query.id])
            else:
                r = self.objects(ObjectFilter().object(obj))
                return r[0] if r else None
        ...
        def flushObject(self, new_object):
            with self.lock:
                ...
                if not old_object:
                    return self.createObject(new_object.displayNameTemplate)
                elif old_object.displayNameTemplate != new_object.displayNameTemplate:
                    with self.transaction() as c:
                        c.execute('update object set display_name = ? where id = ?',
                    ...
                    # we did not change tags, so should update it
                    new_object = old_object.allTags

                    # notify about this object
                    self.objectUpdated.emit(copy(new_object))

                    # and affected tags
                    affected_objects = self._get_affected(new_object)
                    for obj in affected_objects:
                        self.objectUpdated.emit(copy(obj))
        ...
        def createLink(self, obj, tag):
            ...
            with self.lock:
                ...
                with self.transaction() as c:
                    c.execute('insert into links(object_id, tag_class_id, tag_value, tag_id ' \
                              + 'values(?, ?, ?, ?)', (unmod_obj.id, tag.tagClass.id,
                                                         tag.tagClass.valueType, tag.id))
        ...
        def removeLink(self, obj, tag):
            ...
            with self.lock:
                ...
                if not unmod_object.testTag(TagFilter().tag(tag)):
                    return
                ...
                if obj.id in self._objects:
                    actual_tags = unmod_object.allTags
                    actual_tags.remove(self.tag(tag))
                    self._objects.setAllTags(actual_tags)
    ...
    def _checkTag(tag_class, tag_value):
        ...
        if not tag_class.tagClass:
            raise ValueError('invalid tag class for tag')

        if value.valueType == TagValue.TYPE_OBJECT_REFERENCE:
            if not self.object(value.objectReference):
                raise ValueError('invalid reference to object with id #{0}' \
                                    .format(value.objectReference))
    ...
    def _get_affected(obj, watched = None):
